import gym
import numpy as np
import tensorflow as tf
#import tensorflow.compat.v1 as tf
#tf.disable_v2_behavior()


class Policy_net:
    def __init__(self, name: str, env, temp=0.1):
        """
        :param name: string
        :param env: gym env
        :param temp: temperature of boltzmann distribution
        """

        ob_space = env.observation_space
        act_space = env.action_space

        with tf.compat.v1.variable_scope(name):
            self.obs = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None] + list(ob_space.shape), name='obs')

            with tf.compat.v1.variable_scope('policy_net'):
                layer_1         = tf.compat.v1.layers.dense(inputs=self.obs, units=20, activation=tf.tanh)
                layer_2         = tf.compat.v1.layers.dense(inputs=layer_1, units=20, activation=tf.tanh)
                layer_3         = tf.compat.v1.layers.dense(inputs=layer_2, units=act_space.n, activation=tf.tanh)
                self.act_probs  = tf.compat.v1.layers.dense(inputs=tf.divide(layer_3, temp), units=act_space.n, activation=tf.nn.softmax)

            with tf.compat.v1.variable_scope('value_net'):
                layer_1         = tf.compat.v1.layers.dense(inputs=self.obs, units=20, activation=tf.tanh)
                layer_2         = tf.compat.v1.layers.dense(inputs=layer_1, units=20, activation=tf.tanh)
                self.v_preds    = tf.compat.v1.layers.dense(inputs=layer_2, units=1, activation=None)

            self.act_stochastic = tf.random.categorical(tf.math.log(self.act_probs), num_samples=1)
            self.act_stochastic = tf.reshape(self.act_stochastic, shape=[-1])

            self.act_deterministic = tf.argmax(input=self.act_probs, axis=1)

            self.scope = tf.compat.v1.get_variable_scope().name

    def act(self, obs, stochastic=True):
        if stochastic:
            return tf.compat.v1.get_default_session().run([self.act_stochastic, self.v_preds], feed_dict={self.obs: obs})
        else:
            return tf.compat.v1.get_default_session().run([self.act_deterministic, self.v_preds], feed_dict={self.obs: obs})

    def get_action_prob(self, obs):
        return tf.compat.v1.get_default_session().run(self.act_probs, feed_dict={self.obs: obs})

    def get_variables(self):
        return tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES, self.scope)

    def get_trainable_variables(self):
        return tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, self.scope)

